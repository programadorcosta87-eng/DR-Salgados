...
const defaultDB: DB = {
  config: {
    ...
    adminPassword: "1234",     // Simple default password as requested
    isOpen: true,
...
async function startServer() {
  ...
  const authMiddleware = (req: any, res: express.Response, next: express.NextFunction) => {
    ...
    const emp = employees.find(e => e.password === password);
    if (emp) {
  ...
  app.post("/api/admin/config", authMiddleware, requireAdmin, (req, res) => {
    ...
    if (adminPassword !== undefined && adminPassword.trim().length >= 4) {
  ...
  app.put("/api/admin/employees/:id", authMiddleware, requireAdmin, (req, res) => {
    ...
    if (password !== undefined) emp.password = password;
  ...
  app.post("/api/admin/db-restore", authMiddleware, requireAdmin, (req, res) => {
    try {
      ...
      db.config = { ...db.config, ...backupData.config };
      ...
      db.employees = Array.isArray(backupData.employees) ? backupData.employees : db.employees;
